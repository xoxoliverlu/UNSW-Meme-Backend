...
describe('Valid inputs for auth logoutV1', () => {
  test('Successful logout - one session', () => {
    ...
    expect(requestUserProfile(register.token, register.authUserId)).toStrictEqual(403);
  });
  ...
  test('Successful logout - multiple sessions', () => {
    ...
    expect(requestUserProfile(register.token, register.authUserId)).toEqual(403);
    expect(requestUserProfile(login1.token, register.authUserId)).toEqual(403);
    const userDetail = {
      user: {
...
describe('Invalid inputs for authLogoutV1', () => {
  test('Invalid token', () => {
    ...
    expect(logout).toStrictEqual(403);
  });
  test('Logging out of same session twice', () => {
    ...
    expect(logout1).toStrictEqual({});
    expect(logout2).toStrictEqual(403);
    expect(logout3).toStrictEqual({});
    expect(logout4).toStrictEqual(403);
  });
});
