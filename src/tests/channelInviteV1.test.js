...
test("Testing inviter who is not a member of the channel", () => {
    ...
    expect(result).toStrictEqual({ error: "authUserId is not in the channel" });
});
...
test("Testing channel which is non-existent", () => {
    ...
    expect(result).toStrictEqual({ error: "Invalid channelId" });
});
...
test("Testing invitee who is non-existent", () => {
    ...
    expect(result).toStrictEqual({ error: "Invalid uId" });
});
...
test("Testing invitee already a member of channel", () => {
    ...
    let result = channelInviteV1(0, 0, 1);
    expect(result).toStrictEqual({ error: "uId is already in channel" });
}); 