...
describe('Testing messageEdit', () => {
    describe('errors', () => {
        test('length of message is over 1000 characters', () => {
            ...
            const channel = channelsCreate(register.token, 'Hello Kitty', true);
        ...
        test('the message was not sent by the authorised user making this request and the user does not have owner permissions in the channel/DM', () => {
            ...
            expect(data).toStrictEqual({error: 'message was not sent by this user, and user does not have owner permission'});
        });
        test('token is invalid', () => {
            const register = authRegister('dimpi@gmail.com', 'dimpigarnepudi', 'dimpi', 'garnepudi');
            const channel = channelsCreate(register.token, 'Hello Kitty', true);
            const message = messageSemd(register.token, channel.channelId, 'cat');
            expect(message).toStrictEqual({messageId: message.messageId});
            const edit = 'dog';
    ...
    describe('errors', () => {
        test('user that sent message in channel, edits message', () => {
            ...
            const channel = channelsCreate(register.token, 'Hello Kitty', true);
            channelInvite(register.token, channel.channelId, register2.authUserId);
            const message = messageSend(register2.token, channel.channelId, 'cat');
            const edit = 'dog';
            const data = messageEdit(register2.token, channel.channelId, 0);
        ...
        test('user with owner perms in channel edits messages', () => {
            const register = authRegister('dimpi@gmail.com', 'dimpigarnepudi', 'dimpi', 'garnepudi');
            const register2 = authRegister('dimpigarnepudi@gmail.com', 'dimpidimpidimpi', 'dimpi', 'garnepudi');
            const channel = channelsCreate(register.token, 'Hello Kitty', true);
            channelInvite(register.token, channel.channelId, register2.authUserId);
        ...
        test('user with owner perms in DM edits message', () => {
            const register = authRegister('dimpi@gmail.com', 'dimpigarnepudi', 'dimpi','garnepudi');
            const register2 = autheRegister('dimpigarnepudi@gmail.com','dimpidimpidimpi', 'dimpi', 'garnepudi');
            const channel = channelsCreate(register.token, 'Hello Kitty', true);
            channelInvite(register.token, channel.channelId, register2.authUserId);
            const message = messageSend(register2.token, channel.channelId, 'cat');
    ...
    test('user that sent message in DM, edits message', () => {
        ...
        const message = messageSendDm(register2.token, dm.dmID, 'cat');
        const edit = 'cat';
      const data = messageEdit(register2.token, message.messageId, edit);
      expect(data).toStrictEqual({});
