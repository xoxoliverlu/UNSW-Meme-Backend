...
describe('Testing channelMessages1, errors', () =>{
  test('the channelId does not refer to a valid channel', () => {
    const registerAcc = authRegisterV2('jayjay123@gmail.com', 'jayjay123', 'Jayden', 'Jacobs');
    const channelCreationValid = channelsCreateV2(registerAcc.token, 'jaydensChannel', true);
  ...
  test('the user is not a member of the channel', () => {
    const registerAcc = authRegisterV2('jayjay123@gmail.com', 'jayjay123', 'Jayden', 'Jacobs');
    const registerAcc_2 = authRegisterV2('treehouse@gmail.com', 'qwerty', 'Tree', 'House');
    const channelCreationValid = channelsCreateV2(registerAcc.token, 'jaydensChannel', true);
