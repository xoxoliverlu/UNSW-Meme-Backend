...
describe('Testing messagesendv1', () => {
    test('messagesendv1 success', () => {
      const register = authRegister('dimpi.garnepudi@gmail.com', 'dimpi123', 'Dimpi', 'Garnepudi');
    ...
    test('more messages', () => {
      const register = authRegister('dimpi.garnepudin@gmail.com', 'dimpi123', 'Dimpi', 'Garnepudi');
      const channel = channelsCreate(register.token, 'channelNew', true);
      messageSend(register.token, channel.channelId, 'cat');
      const message2 = messageSend(register.token, channel.channelId, 'hellobear');
      expect(message2).toStrictEqual({ messageId: message2.messageId });
    });
  
    test('channelId does not belong to a valid channel', () => {
      const register1 = authRegister('dimpi.garnepudi@gmail.com', 'dimpi123', 'Dimpi', 'Garnepudi');
